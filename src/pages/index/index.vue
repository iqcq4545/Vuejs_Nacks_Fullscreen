<template>
  <div class="page fullpage">
    ...
    <div class="left">
      <div class="left0 bar">
        <h2 class="top up">近一个月整改完成情况</h2>
        <div class="body">
          <p v-if="noData.left[0]" class="noData">暂无数据</p>
          <div
            v-show="!noData.left[0]"
            id="bar0"
            class="bar0"
            style="width:2.05rem;height:1.02rem;position:relative"
          ></div>
        </div>
      </div>
      ...
      <div class="middle0">
        ...
        <ul class="main2">
          <li>
            ...
            <p class="fwb">
              <span v-for="(item,i) in fmtNum(count.powerOn)">{{item}}</span>
            </p>
          </li>
          ...
          <li>
            ...
            <p class="fwb">
              <span v-for="(item,i) in fmtNum(count.warning)">{{item}}</span>
            </p>
          </li>
          ...
          <li>
            ...
            <p class="fwb">
              <span v-for="(item,i) in fmtNum(count.monitor)">{{item}}</span>
            </p>
          </li>
        </ul>
    ...
    <div class="right">
      ...
      <div class="right1 pie">
        ...
        <div class="body">
          ...
          <div
            v-show="!noData.right[1]"
            id="pie"
            class="pie"
            style="width:2.05rem;height:.85rem;position:relative"
          ></div>
        </div>
      </div>
      ...
      <div class="right2">
        ...
        <div class="body">
          ...
          <div
            v-show="!noData.right[2]"
            id="line"
            style="width:2.05rem;height:1rem;position:relative"
          ></div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    requestData() {
      ...
      that.$ReqScreen.getDeviceCheck().then(res => {
        that.deviceCheck = that.parseData(res.data.data, [
          "DeviceName",
          "CheckCount"
        ]);
        that.noData.right[2] = !(
          that.deviceCheck.x.length + that.deviceCheck.y.length
        );
      ...
      that.$ReqScreen.getErrorCount().then(res => {
        that.errorCount = that.parseRose(res.data.data, [
          "FaultType",
          "RepairCount"
        ]);
        ...
        ? that.Toast(
            {
              text:
                "载入中 " +
                Math.floor(
                  (that.pollingCount.complete / that.pollingCount.total) * 100
                ) +
                "%",
              icon: "loading"
            },
        ...
        series: [
          {
            ...
            data: [
              ...
              {
                value:
                  data[0] === "今日报修" ? data[1] / 2 : that.countJob.sum / 2,
        ...
        if (that.tbody.length > 1) {
          for (var i = 0; i < that.tbody.length; i++) {
            ...
            if (tr.length > row[i]) {
              ...
              //判断是否到底，如果到底就返回顶部
              if (
                tbody.scrollTop + tbody.offsetHeight >
                tr_height * tr.length - tr_height / 15
              ) {
                tbody.scrollTop = 0;
              }
              //每次向上滚动5%
